...
# ============================================================
# PHASE 2R -- HARDER VARIANT
# GRAPH-READY WLS + PROTECTED-PRIOR TELEMETRY
# ============================================================
#
# Combines both untested stress levers from Phase 2S's own prescription
# on top of the HARD attack magnitude (21_..._HARD.py):
#   - attack magnitude: already reduced ~3x (inherited from HARD)
#   - load-forecast uncertainty: 2.5% sigma / +-6% clip -> 5.0% / +-12%
# (The third lever -- restricting the compromised-measurement subset --
# is not a free parameter of the model-consistent attack: it's a
# physical consequence of which measurements the perturbed state
# actually touches, so it isn't varied here.)
#
...
def prior_context_from_actual(context, rng):
    ...
    ea = float(np.clip(rng.normal(0.0, 0.05), -0.12, 0.12))
    eb = float(np.clip(rng.normal(0.0, 0.05), -0.12, 0.12))
...
def plot_target_vs_nontarget(node_df):
    ...
    fig.tight_layout()
    fig.savefig(
        FIGURES / "48_graph_prior_innovation_target_vs_nontarget_harder.png",
        dpi=220,
    )
    plt.close(fig)
...
def plot_localization(diag):
    ...
    fig.tight_layout()
    fig.savefig(
        FIGURES / "49_residual_vs_prior_localization_harder.png",
        dpi=220,
    )
    plt.close(fig)
...
def plot_stealth_relation(node_df):
    ...
    fig.tight_layout()
    fig.savefig(
        FIGURES / "50_stealth_topology_footprint_harder.png",
        dpi=220,
    )
    plt.close(fig)
...
def main():
    ...
    print("Prior load forecast sigma : 5.0% per load, clipped to +/-12% (HARDER)")
    ...
    node_df.to_csv(
        DATA / "phase2r_harder_graph_node_features.csv",
        index=False,
    )
    edge_df.to_csv(
        DATA / "phase2r_harder_graph_edge_features.csv",
        index=False,
    )
    graph_df.to_csv(
        DATA / "phase2r_harder_graph_metadata.csv",
        index=False,
    )
    summary.to_csv(
        RESULTS / "phase2r_harder_graph_telemetry_summary.csv",
        index=False,
    )
    diag.to_csv(
        RESULTS / "phase2r_harder_localization_diagnostic.csv",
        index=False,
    )
    ...
    print("\nSaved:")
    print("  data/phase2r_harder_graph_node_features.csv")
    print("  data/phase2r_harder_graph_edge_features.csv")
    print("  data/phase2r_harder_graph_metadata.csv")
    print("  results/phase2r_harder_graph_telemetry_summary.csv")
    print("  results/phase2r_harder_localization_diagnostic.csv")
    print("  figures/48_graph_prior_innovation_target_vs_nontarget_harder.png")
    print("  figures/49_residual_vs_prior_localization_harder.png")
    print("  figures/50_stealth_topology_footprint_harder.png")
