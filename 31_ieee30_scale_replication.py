...
def one_replication(rep, load_buses, node_ids, adjacency, rng, base_net):
    ...
    for case_name in CASES:
        ...
        # this used to be np.abs(vm_hat - vm_true) * 100.0 -- vm_true is the
        # simulator's hidden ground-truth state, never available to a real
        # detector. measurement_schema() (phase21) puts all n_bus voltage
...
def main():
    ...
    print(f"\n=== PHASE 2X: IEEE 30-BUS SCALE REPLICATION ===")
    ...
    DATA.mkdir(exist_ok=True)
    df.to_csv(DATA / "phase2x_ieee30_feature_matrix.csv", index=False)
    node_df.to_csv(DATA / "phase2x_ieee30_node_feature_matrix.csv", index=False)
    ...
    det_df.to_csv(RESULTS / "phase2x_ieee30_detection_metrics.csv", index=False)
    loc_df.to_csv(RESULTS / "phase2x_ieee30_localization_metrics.csv", index=False)
    print("\nSaved:\n  results/phase2x_ieee30_detection_metrics.csv"
          "\n  results/phase2x_ieee30_localization_metrics.csv"
          "\n  data/phase2x_ieee30_feature_matrix.csv"
          "\n  data/phase2x_ieee30_node_feature_matrix.csv")
