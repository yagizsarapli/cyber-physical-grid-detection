...
\caption{5-bus vs.\ IEEE 14-bus vs.\ IEEE 30-bus, all 16-seed means, all four feature sets. The gap is network-specific, not task-specific: null on both tasks at 5-bus, a small but mostly statistically confirmed advantage on both tasks at IEEE 14-bus and IEEE 30-bus (Table~\ref{tab:gapstats}).}
\label{fig:scale}
\end{figure}
...
\smallskip
\footnotesize Sign test excludes exact ties. A Bonferroni correction for 6 simultaneous tests needs $p<0.0083$ (family-wise 95\%): three of the four positive gaps clear it; IEEE-14 detection ($p=0.011$) does not.
\end{table}
...
\subsection{Statistical Confirmation at 16 Seeds}
...
Read together, Sections~\ref{sec:results1}--\ref{sec:scale} converge on a pattern that is network-specific rather than task-specific, once the seed count is large enough to separate a genuinely small effect from per-seed noise (Section~\ref{sec:scale-stats}). At the small custom 5-bus microgrid, explicit topology-relational features show no advantage over a matched non-relational ablation on either detection (mean $-0.000\pm0.009$, 16 seeds) or localization (mean $+0.003\pm0.017$). At both standard IEEE test systems, the same ablation shows a small, mostly confirmed positive gap on both tasks: IEEE 14-bus detection ($+0.009\pm0.012$) and localization ($+0.018\pm0.017$); IEEE 30-bus detection ($+0.028\pm0.019$) and localization ($+0.013\pm0.016$). Put plainly: the two standard IEEE test systems behave alike, and differently from the small custom microgrid, on both tasks -- not, as an original four-seed pilot appeared to show, a different single task favored at each network.
