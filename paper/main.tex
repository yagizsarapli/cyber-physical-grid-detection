...
\begin{table}[t]
\centering
\caption{5-bus vs.\ IEEE 14-bus vs.\ IEEE 30-bus: best result per task, at the same $n{=}300$-test-scenario size throughout (bal.\ acc.\ / top-1)}
\label{tab:scale}
\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
...
Det., IEEE-30 (4-seed mean) & 0.756 & 0.754 & 0.709 & 0.750 \\
Loc., 5-bus & 0.980 & \textbf{0.993} & 0.780 & 0.700 \\
Loc., IEEE-14 (4-seed mean) & \textbf{0.553} & 0.532 & 0.528 & 0.532 \\
Loc., IEEE-30 (4-seed mean) & 0.532 & 0.513 & 0.515 & 0.518 \\
\bottomrule
\end{tabular}

\smallskip
\footnotesize IEEE-14, confirmed across 4 independent seeds (Table~\ref{tab:ieee14seeds}): for detection, \texttt{topology\_fusion} trails the best non-relational alternative each seed (gap $-0.015\pm0.004$, sign never flips); for localization, it leads each seed (gap $+0.018\pm0.010$, sign never flips either way) -- both directions as seed-robust as the 5-bus result, just smaller in magnitude. IEEE-30, re-tested across the same 4 seeds (Table~\ref{tab:ieee30seeds}) after an initial single run appeared to match IEEE-14's pattern, does \emph{not} replicate it: the detection gap changes sign twice ($-0.007$ to $+0.013$) and the localization gap changes sign once ($-0.020$ to $+0.020$) across the 4 seeds -- neither is distinguishable from zero at this replication count, unlike IEEE-14's uniformly-signed gaps. IEEE-30's rows above are left unbolded for this reason: the apparent column-mean leader is not a seed-robust winner.
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../figures/paper_fig4_scale_comparison.png}
\caption{5-bus vs.\ IEEE 14-bus vs.\ IEEE 30-bus, all 4-seed means, all four feature sets. Detection: \texttt{topology\_fusion} never confidently ahead, at any scale. Localization: the topology-free ablation wins decisively at 5-bus; \texttt{topology\_fusion} keeps a small, seed-robust lead at IEEE-14 only (Table~\ref{tab:ieee14seeds}) -- IEEE-30's own 4-seed replication (Table~\ref{tab:ieee30seeds}) does not show the same lead, so the reversal is confirmed at one scale, not (yet) two.}
\label{fig:scale}
\end{figure}
...
\textbf{A third network, replicated: the single-run corroboration does not hold up.} The same pipeline, re-run unmodified on IEEE 30-bus (\texttt{pandapower.networks.case30}, confirmed compatible with \texttt{h\_ac()} the same way IEEE 14-bus was -- \texttt{net.bus} count matches the internal power-flow-case bus count, so no CIGRE-MV-style indexing break), was first run at a single seed (20260921, this project's own primary seed throughout) and appeared to reproduce both halves of the IEEE-14 result closely: detection gap $-0.007$, localization gap $+0.020$. Given this project's own repeated lesson about trusting single-run, small-margin results -- the identical caution that motivated Table~\ref{tab:ieee14seeds} in the first place -- we re-ran it across the same 4 seeds used at IEEE-14 (Table~\ref{tab:ieee30seeds}) before accepting that corroboration.
...
\begin{enumerate}
\item \textbf{Scale}: Section~\ref{sec:scale}'s IEEE 14-bus study independently cross-validates Section~\ref{sec:results1}'s corrected 5-bus \emph{detection} finding using an unrelated feature-engineering design (itself independently found to need, and given, the same correction -- Section~\ref{sec:scale}), now confirmed across 4 seeds (Table~\ref{tab:ieee14seeds}), not just one run. \emph{Localization} does not cross-validate the same way -- it shows a genuine, 4-seed-robust reversal instead (topology-free wins at 5-bus, \texttt{topology\_fusion} wins narrowly at IEEE-14) -- which we read as a real scale-dependent effect, not noise. IEEE 30-bus, re-tested across the same 4 seeds after an initial single run appeared to corroborate IEEE-14, does not replicate that pattern: neither gap holds a consistent sign there (Table~\ref{tab:ieee30seeds}). Three network topologies have now been tested to the same 4-seed standard; the localization reversal is confirmed at exactly one of them (IEEE-14), a null result at the other two (5-bus, IEEE-30); none uses a real (non-synthetic) network or load profile, and whether IEEE-30's null result reflects a genuinely IEEE-14-specific effect or simply needs more replications per seed to resolve is open.
...
Across a small custom inverter-dominated microgrid and two standard, larger, meshed IEEE test systems (14-bus and 30-bus), explicit topology-relational features show no measurable detection advantage over a fair, same-information ablation (residual and prior-innovation features combined) -- a finding reached independently at all three scales, with two unrelated feature-engineering implementations, and confirmed across four independent seeds at every scale tested. For localization, the honest result is a genuine, replicated reversal at exactly one of the three scales, not a general property of larger or standard networks: the topology-free ablation wins decisively on the small custom network, and \texttt{topology\_fusion} keeps a small but seed-robust advantage at IEEE 14-bus -- but an equally careful 4-seed replication at IEEE 30-bus, which an earlier single run had seemed to corroborate, shows no consistent-sign effect in either direction. We take this as a caution against generalizing a two-scale pattern into a network-type rule from a single additional data point, and as a third instance, within this same project, of an unreplicated single-run result not surviving the replication standard the project's own earlier corrections established. Neither the uniform detection result nor the scale-specific localization one was the project's starting hypothesis: an initial, multi-seed-validated positive result (\texttt{topology\_fusion} beating \texttt{prior\_only} alone, at 5-bus, on both tasks) was real, but built on an incomplete comparison, exposed only once a genuinely topology-free control condition was constructed and a feature-labeling bug it revealed was corrected -- independently, in two separate pipelines. Separately, the resulting decision pipeline can be brought within a one-cycle protection-relevant computational target once its actual bottleneck -- a software inefficiency, not the model or the estimator's numerics -- is found and fixed, and the detector does not extend to attack types unseen in training. We think the corrected, mostly-negative-but-not-uniformly-negative version of this result is more useful, and more publishable, than either an unqualified positive or an unqualified null result would have been: it identifies a specific methodological trap (comparing an information-rich feature set against baselines that were not actually information-poor) that is easy to fall into and cheap to check for, and it shows that trap does not always lead to the same answer once corrected -- topology-awareness's value, on this evidence, is task- and scale-specific, not simply present or absent.
