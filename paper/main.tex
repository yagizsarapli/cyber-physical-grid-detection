...
Detection shows a small edge for \texttt{topology\_fusion} at this one seed, in both conditions ($+0.013$ Hard, $+0.003$ Harder); localization is an exact tie under Hard and a small edge for \texttt{residual\_plus\_prior} under Harder ($-0.007$). Neither column is bolded here -- a single seed's small, direction-inconsistent gap is not evidence of a real winner, and the 16-seed replication directly below (which includes this same seed) is the test that actually matters. For reference, both feature sets still comfortably beat \texttt{prior\_only} alone (0.860--0.870 detection, 0.687--0.693 localization) and \texttt{residual\_only} alone (0.700 detection, 0.547 localization) -- the gain over either single baseline is large; it is simply not a topology effect.
