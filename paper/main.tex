...
\textbf{Contributions:}
\begin{itemize}
\item A methodologically rigorous negative result: once an (independently rediscovered, twice) feature-labeling bug is corrected and a fair, same-information ablation is built, explicit topology-relational features measure no benefit over combining residual and prior-innovation features for cyber-physical event detection or localization -- confirmed across two stress conditions and four independent seeds at one network scale, and cross-validated at a second, larger scale with an unrelated feature-engineering design that needed the identical fix (Section~\ref{sec:results1}, Section~\ref{sec:scale}), together with a structural, quantitative explanation for why (Section~\ref{sec:results1}).
...
Det., IEEE-14 & 0.737 & 0.743 & 0.697 & \textbf{0.750} \\
Loc., 5-bus & 0.980 & \textbf{0.993} & 0.780 & 0.700 \\
Loc., IEEE-14 & \textbf{0.560} & 0.533 & 0.547 & 0.547 \\
\bottomrule
\end{tabular}

\smallskip
\footnotesize IEEE-14 detection: \texttt{residual\_only}/LR leads (0.750), with \texttt{residual\_plus\_prior} (0.743) and \texttt{topology\_fusion} (0.737) close behind -- neither the ablation nor topology helps. IEEE-14 localization: \texttt{topology\_fusion} (0.560) is narrowly ahead of every non-relational alternative (0.533--0.547) -- a single run, not multi-seed-confirmed at this scale, unlike the 5-bus column.
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../figures/paper_fig4_scale_comparison.png}
\caption{5-bus vs.\ IEEE 14-bus (both at $n{=}300$ test scenarios), all four feature sets at both scales. No feature set, topology-inclusive or not, is consistently ahead of the topology-free \texttt{residual\_plus\_prior} ablation at either scale.}
\label{fig:scale}
\end{figure}

\textbf{Result, three escalating passes, same direction each time} (Table~\ref{tab:scale}, Fig.~\ref{fig:scale}; replication counts per the terminology defined in Section~\ref{sec:method}). The first two passes -- 80 replications ($\sim$24 test scenarios) and 200 replications (60--120 test scenarios) -- predate the correction above and compared only the original (uncorrected) \texttt{residual\_only}/\texttt{prior\_only}/\texttt{topology\_fusion}; both were inconclusive-to-suggestive purely on sample-size grounds, the same underpowered regime that had already produced, and been corrected out of, a false reversal in Section~\ref{sec:results1} -- a statement about $N$, not about the feature-definition fix that came later. The final, corrected pass at 500 replications -- yielding a 300-scenario test set, matched exactly to Section~\ref{sec:results1}'s own -- is what Table~\ref{tab:scale} reports: residual-only detection is the single best result overall (0.750), with \texttt{residual\_plus\_prior} (0.743) and \texttt{topology\_fusion} (0.737) close behind in that order; for localization, \texttt{topology\_fusion} (0.560) is narrowly ahead of every non-relational alternative (0.533--0.547), a margin this single run cannot distinguish from noise without the kind of multi-seed replication Section~\ref{sec:results1} ran at 5-bus.

\textbf{Interpretation: two independently-built pipelines, corrected independently, agree.} Section~\ref{sec:results1} (5-bus) and this section (IEEE 14-bus) were built with unrelated feature-engineering code -- regex-derived fixed per-bus columns vs.\ size-agnostic global summaries -- and, caught only during this audit, each carried its own version of the same conceptual flaw, fixed separately above and in Section~\ref{sec:method}. With both corrected, the two sections no longer read as ``does topology help'' and ``does that transfer to scale'' -- they read as the same question, asked and answered twice with independent code: explicit topology-relational features carry no confidently-attributable advantage over a fair, non-relational baseline, at either scale. That the same category of comparison mistake was made independently in two separately-written pipelines is itself informative -- it suggests this is an easy trap when building a ``topology-aware'' feature set generally, not a one-off slip specific to one script -- and that both, once corrected, land on the same qualitative answer is stronger evidence than either alone. That said, the IEEE-14 localization margin (0.560 vs.\ 0.533--0.547) is narrow and under-replicated (one run, no seeds, no bootstrap CI) and should not be read as decisive either way, unlike the seed- and stress-condition-robust 5-bus result. This is consistent with, and sharpens, a 2025 field-wide scoping review's \cite{oelhaf2025scoping} independent finding that cross-topology generalization is essentially untested across the ML-for-power-protection literature it surveyed -- this study is a direct, sample-size-matched, cross-implementation attempt at a gap the field has already named, not a self-inflicted complication and not a preliminary pilot.

\section{Discussion}

Read together, Sections~\ref{sec:results1}--\ref{sec:scale} converge on one finding, reached twice, independently, in two separately-written pipelines that each turned out to need the same category of fix: \emph{explicit topology-relational features do not measurably improve cyber-physical event detection or localization on this class of problem, once measured against a fair, same-information ablation} -- not at a small 5-bus network, and not at a larger, meshed IEEE 14-bus network built with an unrelated feature design. What does help, considerably, is combining residual and prior-innovation information: a real, multi-seed-confirmed $\sim$0.09--0.21 gain over either alone (Section~\ref{sec:results1}), just not a topology-specific one. Section~\ref{sec:results1}'s redundancy analysis gives this a structural, not just empirical, footing at the smaller scale: on a fixed, small topology, a relational aggregate is close to a linear function of information a flexible classifier already has, so there is little left for an explicit relational feature to contribute. This also sharpens the paper's remaining open question -- not ``does topology help,'' which this paper answers no to twice, but ``on what kind of network would relational aggregates carry information that is \emph{not} already linearly recoverable from local features,'' which Section~\ref{sec:results1}'s method (an out-of-fold redundancy check, cheap to run on any candidate network before training a single classifier) can test directly, before running the full detection/localization pipeline at all.
...
\begin{enumerate}
\item \textbf{Scale}: Section~\ref{sec:scale}'s IEEE 14-bus study independently cross-validates Section~\ref{sec:results1}'s corrected 5-bus finding using an unrelated feature-engineering design (itself independently found to need, and given, the same correction -- Section~\ref{sec:scale}), but only two network topologies have been tested overall, and neither uses a real (non-synthetic) network or load profile. The IEEE-14 localization result specifically (\texttt{topology\_fusion} narrowly ahead, Table~\ref{tab:scale}) is a single run with no seed replication or bootstrap CI, unlike the 5-bus result, and should be read as suggestive, not confirmed.
