...
Detection, IEEE-14 ($n{=}500$) & \textbf{residual\_only/LR 0.750} -- outright ahead of topology\_fusion's own best (0.737) \\
Localization, 5-bus & \textbf{topology\_fusion 0.980} \\
Localization, IEEE-14 ($n{=}500$) & topology\_fusion 0.560 -- narrowly best again, but tight (prior\_only 0.547, residual\_only 0.507) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Result, three escalating passes, same direction each time} (Table~\ref{tab:scale}, Fig.~\ref{fig:scale}). $n{=}80$ ($\sim$24 test scenarios) was inconclusive by design -- the same underpowered regime that had already produced, and been corrected out of, a false reversal in Section~\ref{sec:results1}. $n{=}200$ (60--120 test scenarios) first showed an internally consistent pattern across detection and localization rather than model-to-model noise. $n{=}500$ -- matched exactly to Section~\ref{sec:results1}'s own 300-scenario test-set size -- did not walk that back; it sharpened it: residual-only detection is now the outright leader on IEEE 14-bus, not merely competitive. This rules out $n{=}200$ having itself been an underpowered artifact in the other direction; the finding is stable under the same statistical rigor applied to the headline result.
...
\begin{enumerate}
\item \textbf{Scale}: the validated headline result (Section~\ref{sec:results1}) is a 5-bus network. The scale study (Section~\ref{sec:scale}, IEEE 14-bus, confirmed at $n{=}500$, matched to Section~\ref{sec:results1}'s own power) complicates rather than confirms it. Only two network topologies have been tested; a third standard system (e.g.\ IEEE 30-bus) would show whether this pattern itself generalizes.
