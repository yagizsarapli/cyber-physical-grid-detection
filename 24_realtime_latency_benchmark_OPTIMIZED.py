...
# ============================================================
# PHASE 2T -- OPTIMIZED (post-diagnosis)
# END-TO-END DECISION LATENCY VS. PROTECTION-GRADE BUDGET
# ============================================================
#
# 25_wls_overhead_diagnostic.py plus a direct split of run_wls()
# found the original ~40ms was NOT dominated by the WLS numerical
# solve alone -- it was roughly a 50/50 split:
#   - add_measurement_vector(): ~20ms. This loops pp.create_measurement()
#     once per channel (23x/cycle), rebuilding net.measurement's
#     pandas rows from scratch every cycle. Pure API-usage overhead,
#     not numerical work: the measurement TABLE STRUCTURE is
#     identical every cycle, only the values change.
#   - estimate(): ~19-20ms, confirmed NOT iteration-count-driven
#     (5 vs 40 max_iterations: same speed) -- internal pandapower
#     overhead we don't control from the outside.
#
# Fix applied here: build the measurement table ONCE (one-time setup)
# and overwrite its 'value' column in place each cycle instead of
# recreating all 23 rows. Verified bit-identical WLS output vs the
# original per-call loop (max |Δvm|=0, max |Δva|=0), 1158x faster
# for that sub-step alone. estimate() itself is left untouched.
#
# Purpose
# -------
...
def main():
    ...
    # One-time: build the measurement table structure once (23 rows,
    # fixed name/type/element/std_dev/side). Per cycle we only ever
    # overwrite the 'value' column below -- see header note.
    ...
    summary.to_csv(
        RESULTS / "phase2t_latency_benchmark_optimized.csv", index=False
    )
    print("\nSaved:\n  results/phase2t_latency_benchmark_optimized.csv")
